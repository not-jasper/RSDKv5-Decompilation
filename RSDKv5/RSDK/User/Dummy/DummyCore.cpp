...
DummyCore *InitDummyCore()
{
    ...
    if (achievements) delete achievements;
    achievements = new DummyAchievements;
    if (leaderboards) delete leaderboards;
    leaderboards = new DummyLeaderboards;
    if (richPresence) delete richPresence;
    richPresence = new DummyRichPresence;
    if (stats) delete stats;
    stats = new DummyStats;
    if (userStorage) delete userStorage;
...
int32 DummyCore::GetDefaultGamepadType()
{
    ...
    switch (platform) {
        ...
        case PLATFORM_XB1:
        case PLATFORM_PC:
        case PLATFORM_DEV: return (DEVICE_API_NONE << 16) | (DEVICE_TYPE_CONTROLLER << 8) | (0 << 0);
    }
}
#endif
